(function () {
  ...
  function XmlDocument(xml) {
    ...
    this.parser = new sax.SaxesParser({
      xmlns: true,
      position: true,
      fragment: true
    });
  ...
  function addParserEvents(parser) {
    parser.on("opentag", parser_opentag);
    parser.on("closetag", parser_closetag);
    parser.on("text", parser_text);
    parser.on("cdata", parser_cdata);
    parser.on("comment", parser_comment);
    parser.on("doctype", parser_doctype);
    parser.on("error", parser_error);
  }
